...
const PROMPT = `Analise este relatório financeiro e extraia em JSON: { valor: 'R$ 0,00', descricao: 'breve resumo de uma linha do que o relatório representa' }. O campo "valor" deve ser o valor total/líquido do relatório. Retorne APENAS o JSON, sem texto adicional.`

export async function POST(request: Request) {
  const apiKey = process.env.ANTHROPIC_API_KEY
  if (!apiKey) {
    return NextResponse.json(
      { error: 'ANTHROPIC_API_KEY não configurada no servidor. Adicione a chave no arquivo .env.local para usar a leitura automática de relatórios.' },
      { status: 500 },
    )
  }
  ...
  try {
    ...
    let parsed: { valor?: string | null; descricao?: string | null }
    try {
