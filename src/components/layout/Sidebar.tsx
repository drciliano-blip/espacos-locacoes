...
const navItems = [
  ...
  { href: '/pagamentos', label: 'Pagamentos', icon: CreditCard, page: 'pagamentos' },
  { href: '/contratos', label: 'Contratos', icon: FileText, page: 'contratos' },
...
const roleBadgeColors: Record<NivelAcesso, string> = {
  ...
  financeiro: 'bg-sky-500/15 text-sky-400',
  operacional: 'bg-emerald-500/15 text-emerald-400',
  visualizador: 'bg-zinc-500/15 text-zinc-400',
}
  ...
  return (
    <aside className="flex h-screen w-64 flex-col bg-app-surface border-r border-app-border">
      ...
      <nav className="flex-1 overflow-y-auto px-3 py-4 space-y-1">
        {navItems.filter(item => permissions.includes(item.page)).map(({ href, label, icon: Icon }) => {
        ...
        {/* Espaços section — only for roles with access */}
